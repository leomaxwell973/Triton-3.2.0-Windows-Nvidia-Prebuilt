...
def libcuda_dirs():
    """
    Finds directories containing `cuda.dll` or `nvcuda.dll` (Windows equivalent of `libcuda.so`).
    """
    env_libcuda_path = os.getenv("TRITON_LIBCUDA_PATH")
    if env_libcuda_path:
        return [env_libcuda_path]

    # Default to CUDA_LIB64 location
    if os.path.exists(os.path.join(CUDA_LIB64, "cuda.lib")):
        return [CUDA_LIB64]

    # Try checking if `nvcuda.dll` is available in system directories
    system32_path = os.path.join(os.getenv("SystemRoot", r"C:\Windows"), "System32")
    if os.path.exists(os.path.join(system32_path, "nvcuda.dll")):
        return [system32_path]

    # Fall back to checking PATH
    env_path = os.getenv("PATH", "").split(";")
    for path in env_path:
        if os.path.exists(os.path.join(path, "nvcuda.dll")):
            return [path]

    msg = "CUDA library not found! Please check your CUDA installation."
    raise RuntimeError(msg)

@functools.lru_cache()
def library_dirs():
    """
    Returns library directories, ensuring compatibility with Windows-style paths.
    """
